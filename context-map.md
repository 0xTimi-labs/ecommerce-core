...
- **订单 [U, OHS, PL] → 支付 [D, ACL]**
    - **事件**：发布 `OrderPlaced`，支付上下文订阅并初始化支付单
    - **事件**：发布 `OrderCancelled`，支付上下文订阅并释放预授权/退款
    - **查询**：支付上下文通过 gRPC API 查询订单详情与金额
- **订单 [U, OHS, PL] → 库存履约 [D, ACL]**
    - **事件**：发布 `OrderPlaced`，库存上下文订阅并执行预留（ReserveStock）
    - **事件**：发布 `OrderCancelled`，库存上下文订阅并释放预留库存
- **支付 [U, PL] → 订单 [D, ACL]**
    - **事件**：发布 `PaymentAuthorized` / `PaymentCaptured`，订单上下文订阅推进至 `Paid`
    - **事件**：发布 `PaymentFailed`，订单上下文订阅推进至 `PaymentFailed`
- **库存履约 [U, PL] → 订单 [D, ACL]**
    - **事件**：发布 `StockReserved`，订单上下文订阅推进履约准备
    - **事件**：发布 `StockShortage`，订单上下文触发取消与通知补偿
- **订单 ↔ 支付 ↔ 库存履约 [SK]**
    - **共享内核 (Shared Kernel)**：`crates/shared-kernel` 共享 `OrderId`, `CustomerId`, `SkuId`, `Money` 基础类型
