...
pub mod inventory_service_client {
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + std::marker::Send,
    {
        ...
        > {
            ...
            req.extensions_mut()
                .insert(GrpcMethod::new("inventory.v1.InventoryService", "Reserve"));
        ...
        > {
            ...
            let codec = tonic_prost::ProstCodec::default();
            let path = http::uri::PathAndQuery::from_static(
                "/inventory.v1.InventoryService/Release",
            );
            let mut req = request.into_request();
            req.extensions_mut()
                .insert(GrpcMethod::new("inventory.v1.InventoryService", "Release"));
            self.inner.unary(req, path, codec).await
        }
    }
}
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/inventory.v1.InventoryService/Reserve" => {
                    ...
                    struct ReserveSvc<T: InventoryService>(pub Arc<T>);
                    impl<
                        T: InventoryService,
                    ...
                    for ReserveSvc<T> {
                        ...
                        fn call(
                            ...
                            let fut = async move {
                                <T as InventoryService>::reserve(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
                ...
                "/inventory.v1.InventoryService/Release" => {
                    #[allow(non_camel_case_types)]
                    struct ReleaseSvc<T: InventoryService>(pub Arc<T>);
                    impl<
                        T: InventoryService,
                    ...
                    for ReleaseSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as InventoryService>::release(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
